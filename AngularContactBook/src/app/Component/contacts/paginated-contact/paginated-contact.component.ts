...
@Component({
  selector: 'app-paginated-contact',
  templateUrl: './paginated-contact.component.html',
  styleUrls: ['./paginated-contact.component.css']
})
export class PaginatedContactComponent {
  ...
  ngOnInit(): void {
    ...
    loadContactsCount(): void {
      this.contactService.getAllContactsCountByLetter('').subscribe({
    ...
    loadContactsCountByLetter(letter:string): void {
      ...
      this.contactService.getAllContactsCountByLetter(letter).subscribe({
        next: (response: ApiResponse<number>) => {
          if (response.success) {
            console.log(response.data);
            this.totalItems = response.data;
